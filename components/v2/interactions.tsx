...
import { SystemGlyph } from "./system-art";
import { NexusMark } from "./nexus-mark";
import s from "./interactions.module.css";

function useSelectionMotion(selection: number) {
  ...
  useEffect(() => {
    ...
    if (!changed || reduce.matches || !ref.current) return;
    const animations = [...ref.current.querySelectorAll("h3,p,li,[data-state-graphic]")].map((el, i) => el.animate([
      { opacity: .5, transform: "translateY(6px)" }, { opacity: 1, transform: "none" },
    ], { duration: 320, delay: Math.min(i * 35, 140), easing: "cubic-bezier(.22,1,.36,1)" }));
    const cancel = () => animations.forEach(animation => animation.cancel());
    const preferenceChanged = () => { if (reduce.matches) cancel(); };
    reduce.addEventListener("change", preferenceChanged);
    return () => { cancel(); reduce.removeEventListener("change", preferenceChanged); };
  }, [selection]);
  return ref;
}

export function ResponseSequence({dark=false}:{dark?:boolean}) {
  const [active, setActive] = useState(0);
  const selection = useSelectionMotion(active);
  return <div className={`${s.sequence} ${dark?s.darkSequence:""}`}>
    <div className={s.flowGraphic} aria-hidden="true">{["dispatch","nexus","air","ground","modules","handoff"].map((type,i)=><div key={type} data-current={active===i} data-complete={active>i}><div>{type==="dispatch"?<span>911</span>:type==="handoff"?<span>EMS</span>:<SystemGlyph type={type}/>}</div><span>{["Dispatch","Coordinate","Reach","Access","Extend","Handoff"][i]}</span></div>)}</div>
...
export function NexusConcept() {
  const [active,setActive]=useState(0);
  const selection = useSelectionMotion(active);
  return <div className={s.nexusPanel} ref={selection} data-reveal="graphic">
    <div className={s.nexusBar}><strong><NexusMark /> NEXUS</strong><span>V1 CONCEPT INTERFACE</span><span className={s.notLive}>NOT LIVE</span></div>
    ...
    </div><div className={s.systemStatus} aria-label="Illustrative system review fields"><div><span>AIR</span><strong>{active===2?"Handoff context":"Transport review"}</strong></div><div><span>GROUND</span><strong>{active===2?"Operator control":"Access review"}</strong></div><div><span>MODULE</span><strong>Payload review</strong></div><div><span>SYSTEM HEALTH</span><strong>Review required</strong></div></div><div className={s.nexusFoot}><span>HUMAN-IN-THE-LOOP BY DESIGN</span><span>Concept only · No patient or mission data</span></div>
  </div>;
}
