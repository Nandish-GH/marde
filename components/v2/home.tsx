import { NandishPortrait } from "./nandish-portrait";
...
export function Home() {
  return <div className={s.home} data-v2-home>
    ...
    <section className={`${s.section} ${s.team}`} aria-labelledby="team-heading"><Kicker number="07">People behind the platform</Kicker><div className={s.teamIntro} data-reveal="editorial"><h2 id="team-heading">A shared belief.<br /><span>A developing system.</span></h2><Action href="/team/" secondary>Meet the Team</Action></div><div className={s.people}>{people.map((person,i)=><article key={person.name} className={s.person} data-reveal="person"><div className={s.portrait}>{person.portrait?<NandishPortrait variant="home" />:<><span>{person.initials}</span><i aria-hidden="true">+</i></>}<span className={s.personNumber}>0{i+1}</span></div><h3>{person.name}</h3><span className={s.personRole}>{person.role}</span><p>{person.contribution}</p><p className={s.credential}>{person.credential}</p></article>)}</div><Link href="/mission/" className={s.missionLink}>Why we’re building MARDE<Arrow diagonal /></Link></section>
    ...
    <section className={s.close} aria-labelledby="close-heading"><div className={s.closeGlow} aria-hidden="true" /><Kicker>THE NEXT MILESTONE</Kicker><h2 id="close-heading" data-reveal="editorial">The next step is<br /><span>an integrated V1.</span></h2><p>We’re building Air V1, Ground V1, and Nexus V1 toward an<br className={s.desktopBreak} /> operator-directed, human-in-the-loop demonstration.</p><div className={s.actions}><Action href="/contact/" light>Contact MARDE</Action><Link className={s.quietLink} href="/technology/">Explore the Technology<Arrow diagonal /></Link></div><Link className={s.quietLink} href="/support/">Support MARDE R&amp;D<Arrow diagonal /></Link><span className={s.closeMark} aria-hidden="true">+</span></section>
  </div>;
}
